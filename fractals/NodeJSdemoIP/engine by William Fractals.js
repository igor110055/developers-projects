...
// const API_KEY = ''
// const API_SECRET = ''
const API_KEY = 'oL3kjjfDzylGnoavDAPXDX1A5Vei0QGYXEaJ4UR5U6QRsNP33lCnxm1rtCX4DbwV'
const API_SECRET = '4FySjruZLMAOgrNliTwdYjS0ihWatbp2A5MrrHolTn3aIjgPz5NkM5nLB1RbT6FV'
...
function startProgram() {
  client.getKlines({ symbol: symbol, interval: seniorTimeFrame, limit: limit})
  .then(result => {    
...
async function startProgram2() {
  ...
  try {
    const candlesSenior = await client.getKlines({ symbol: symbol, interval: seniorTimeFrame, limit: limit})
...
function printGlobalProfit(dealsKostil) {
  ...
  console.log(`symbol = ${symbol} | старший ТФ = ${seniorTimeFrame} | младший ТФ = ${lowerTimeFrame}`)
  console.log(`Общая сумма прибыли globalSumProfit = ${+globalSumProfit.toFixed(2)} USD (суммируется внутри функции trade)`)
  //console.log(`Общее кол-во сделок (Длина массива globalDeals) = ${globalDeals.length} шт`)
  console.log(`Общее кол-во сделок (Длина массива dealsGlobal) = ${dealsKostil.length} шт`)
  ...
  console.log(`Максимальная просадка = ${min} USD (${Math.round(min/deposit*10000)/100} %)`)
  ...
  console.log(`итоговая прибыль = ${+sumTest.toFixed(2)} USD (проверка по dealsKostil)`)
  ...
  console.log(`всего сделок = ${countOfPositive + countOfNegative + countOfZero}`)
  ...
  console.log(`время выполнения скрипта = ${diffInSecond} секунд (${diffInSecond/60} минут)`)
...
function findTrends(arg) {
  ...
  for (let i = 4; i < arg.length; i++){
      ...
      if ( (arg[i-4].highPrice < arg[i-2].highPrice) &&
          ...
          (arg[i].highPrice < arg[i-2].highPrice) ){
              ...
              if ((arg[i-4].lowPrice > arg[i-2].lowPrice) &&
...
async function getTrendsAsync(array) {
  ...
  console.log(`общее кол-во трендов = ${array.length} шт (передано в функцию getTrends)`)
  ...
  for (let i = 0; i < array.length; i++) {
    if (i != array.length-1) {
      try {
        ...
        const candlesJunior = await client.getKlines({ symbol: symbol, interval: lowerTimeFrame, startTime: array[i].priceTimeStamp, endTime: array[i+1].priceTimeStamp, limit: 1000})
        const objectJunior = candlesToObject(candlesJunior)
        console.log('')
        console.log(`обработка данных ${i}-го тренда...`)
        console.log(`Длина ${i} тренда составляет ${objectJunior.length} свечей`)
        console.log(`Начало тренда: ${timestampToDateHuman(array[i].priceTimeStamp)}, заверщение тренда: ${timestampToDateHuman(array[i+1].priceTimeStamp)}`)
    ...
    else { // если это последний тренд, то берем текущую последнюю дату
      try {          
        const candlesJunior = await client.getKlines({ symbol: symbol, interval: lowerTimeFrame, startTime: array[i].priceTimeStamp, endTime: currentTime, limit: 1000})
        const objectJunior = candlesToObject(candlesJunior)
        console.log('')
        console.log(`обработка данных ${i}-го тренда...`)
        console.log(`Длина ${i} тренда составляет ${objectJunior.length} свечей`)
        console.log(`Начало тренда: ${timestampToDateHuman(array[i].priceTimeStamp)}, заверщение тренда: ${timestampToDateHuman(currentTime)}`)
...
function trade(array, trend, index) {
  ...
  for (let i = 4; i < array.length; i++) {
      if ((array[i-4].lowPrice > array[i-2].lowPrice) &&
          ...
          if ((array[i-4].highPrice < array[i-2].highPrice) &&
              ...
              if (inLongPosition) {
                  ...
                  if (array[i].lowPrice < stopLoss) {
                      ...
                      deals[numberOfPosition] = {
                          ...
                          percent: +((array[i].lowPrice-positionUp) / positionUp * 100).toFixed(2),
                          stopLoss: stopLoss
                      } 
                  ...
                  if (i == array.length-1) {
                      if (array[i].lowPrice >= lastFractal) {
                          ...
                          deals[numberOfPosition] = {
                              ...
                              percent: +((lastFractal-positionUp) / positionUp * 100).toFixed(2),
                              lf: lastFractal
                          }
                      }
                      lastFractal = 0    
                  ...
                  if (inShortPosition) {
                      ...
                      if (array[i].highPrice > stopLoss) { 
                         // deals[numberOfPosition].push('Buy', Number(array[i][2]), timestampToDateHuman(array[i][0]), +(positionDown - Number(array[i][2])).toFixed(2))
                          ...
                          if (stopLoss > 0) { // бывают ситуации, когда в начале нового тренда верхний фрактал еще не сформировался
                            deals[numberOfPosition] = {
                              openPosition: 'Sell', 
                              ...
                              percent: +((positionDown - array[i].highPrice) / positionDown * 100).toFixed(2),
                              stopLoss: stopLoss
                            } 
                      ...
                      if (i == array.length-1) {
                          if (array[i].highPrice <= lastFractal) {
                              ...
                              deals[numberOfPosition] = {
                                  ...
                                  percent: +((positionDown - lastFractal) / positionDown * 100).toFixed(2),
                                  lf: lastFractal
                              } 
                          }
                          lastFractal = 0    
...
function getTrends(array) {
  ...
  for (let i = 0; i < array.length; i++) {
      ...
      client.getKlines({ symbol: symbol, interval: lowerTimeFrame, startTime: array[i].priceTimeStamp, endTime: array[i+1].priceTimeStamp, limit: 1000}) 
      //client.getKlines({ symbol: symbol, interval: lowerTimeFrame, startTime: 1656669600000, endTime: 1656914400000, limit: 1000})     
         .then(response => {      
          ...
          client.getKlines({ symbol: symbol, interval: lowerTimeFrame, startTime: array[i].priceTimeStamp, endTime: currentTime, limit: 1000})
              .then(response => {        
                  candles = candlesToObject(response)
              })                    
              .then(() => {
...
function printGlobalProfit2(arg) {
  ...
  console.table(dealsGlobal)
  console.log(`длина передаваемого массива в функцию (printValue) = ${dealsGlobal.length}`)
